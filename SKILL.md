...
骨架是跟技术栈绑的、基本不变的**只读素材**,存在独立仓库 `project-skeletons`,含 `python-fastapi/`、`java-springboot/`、`go-standard/`、`c-cmake/`、`rust-cargo/` 等标准结构,每套自带 `make verify` 且开箱绿。

技术架构文档确定语言后,把对应骨架拷进当前项目根:

1. **优先用本机缓存**:若用户本机已 clone 骨架仓库(常见路径如 `~/project-skeletons`,或用户指定的路径),直接从本地拷对应语言子目录,免联网。
2. **否则联网浅拉**:`git clone --depth 1 <骨架仓库地址> /tmp/_skels` → 拷 `/tmp/_skels/<语言>/` 内容进项目根 → 删临时目录。
3. **拉不到(断网/无地址)**:停下来提示用户手动指定本地骨架路径,或先跳过、进第 6 步前补上。别硬编一个空壳糊弄。
